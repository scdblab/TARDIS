...
import edu.usc.bg.utils.CreateResourcesFiles;
...
public class BGCoord {
	...
	public static boolean copyFolder=true;
	...
	public static void main(String args[]) {
		...
		//		System.out.println("Args:"+args[args.length-4]);
		//		System.out.println("Args:"+args[args.length-3]);
		//		System.out.println("Args:"+args[args.length-2]);
		//System.out.println("Num="+numClients+"  " + clientArg);
		...
		if (copyFolder)
		{
			...
			bg_destfolder="\\Core\\KVSJDBC";
			src="\\\\127.0.0.1\\Core\\KVSJDBC";
			copyToClients(src,bg_destfolder,ips);		
			...
			//			copyToClients(src,bg_destfolder);
			//			
			bg_destfolder="\\BG\\RyanListener";
			src="\\\\127.0.0.1\\BG\\RyanListener";
			copyToClients(src,bg_destfolder,ips);
	...
	public void runClients (int numThreads, int numClients, String workload)
	{
		...
		// startup numClients # bgClients
		//	workload = workloadRoot + "\\" + workload;
		//System.out.println("STARTING WORKLOAD " + workload);
		...
		try{
			...
			sv=edu.usc.bg.utils.CreateResourcesFiles.createFiles(path+dir,"listener");
			edu.usc.bg.utils.CreateResourcesFiles.createResourcesFiles(sv,path+dir,"client");
			edu.usc.bg.utils.CreateResourcesFiles.createCharts(path+dir,"client","");
		}
		catch (Exception ex){
		...
		try{
			...
			sv=edu.usc.bg.utils.CreateResourcesFiles.createFiles(path+dir,"os+");
...
class BGThread extends Thread {
	...
	//int delta = Integer.parseInt(CoordinatorDriver.commonProps
	...
	public void initThread() {
		...
		//		String message = listenerClient.getScanner().nextLine();
		//		if (BGCoord.verbose)
		//		System.out.println("From " + listenerClient.getIp()
		//				+ ":" + listenerClient.getPort() + ": " + message);
	...
	public static String createBGRunMessage(int threadID) {
		...
		for (int i=0; i< BGCoord.arguments.length; i++)
		{
			...
			//"-p numclients=2 -p clients=127.0.0.1:53148,127.0.0.1:53128  -p clientid=" +threadID+ " -p nummembers=1000 -p numrequests=1000 -p numthreads=1";
			message=message+ arg+ ' ';
