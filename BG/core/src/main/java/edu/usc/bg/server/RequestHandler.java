		...
		try {
			while (continueHandling) {
				try {
					...
					System.out.println("End of Stream. Good Bye. IP:"+ socket.getSocket().getInetAddress());
				} catch (Exception e) {
...
//					this.continueHandling=true;
				...
				case THREAD_DUMP_REQUEST:
					...
					PrintWriter out = new PrintWriter("C:\\Users\\yaz\\Documents\\BG\\BGServerListenerLogs\\threadDump-"+InetAddress.getLocalHost().getHostAddress()+"-"+System.currentTimeMillis()+".txt");
				...
				case FULL_SHUTDOWN_REQUEST: // full shutdown request from coordinator
					...
					if ( (CoreWorkload.updatesExist && CoreWorkload.readsExist) || ( !CoreWorkload.updatesExist && CoreWorkload.graphActionsExist )  ) {
						...
						boolean printFinalStats=true;
						if (printFinalStats){
							try{
								if (Client.threadCount>0){
							expStat.setOpsTillFirstDeath((double)(int) props.get("allOpsDone"));
							expStat.setActsTillFirstDeath((double) (int)props.get("allActsDone"));
							expStat.setTimeTillFirstDeath( (double) ((long)props.get("firstEn") - Client.experimentStartTime));
							
							Client.printFinalStats(props, (int)props.get("opsDone"), (int)props.get("actsDone"), (long)props.get("en") - Client.experimentStartTime, expStat,
									outpS);
								}
						} catch (Exception e) {
...
//					if (CoreClient.enableCache && 
//							(dbname.toLowerCase().contains("kosarcoreclient") || dbname.toLowerCase().contains("kosarscalabilityclient"))){
