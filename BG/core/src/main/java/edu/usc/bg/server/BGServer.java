...
import edu.usc.bg.testing.SemaphoreMonitorThread;
...
public class BGServer extends Thread {
	...
	public static int NumThreadsPerSemaphore = 10;
	...
	public static ConcurrentHashMap<Integer, ConcurrentLinkedQueue<TokenObject>> requestsToProcess = new ConcurrentHashMap<Integer, ConcurrentLinkedQueue<TokenObject>>(
			NumSemaphores, .75f, 1);
	...
	public BGServer(int semaphores, int workers, int numHandlers, int testcase) {
				...
				while (true) {
					try {
						...
						System.out.println("Error in Server port: "
								+ CLIENT_CONNECT_PORT + " " + e.getMessage());
						if (flag) {
	...
	public static int killBindingProcesses(int port) {
		...
		int numKilled = 0;
		String mypid = ManagementFactory
				.getRuntimeMXBean()
				.getName()
				.substring(
						0,
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (patternLine.contains(pattern)) {
					...
					if (pid.equals(mypid)) {
						System.out.println("Wrong: trying to kill my proces:"
								+ pid);
	...
	public static void main(String[] args) {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (patternLine.contains(pattern1)
						|| patternLine.contains(pattern2)) {
					String[] tokens2 = line.split(" ");
					String pid = tokens2[tokens2.length - 1];
					System.out.println("Killing Process:" + pid);
					// Process p2 = Runtime.getRuntime().exec(
