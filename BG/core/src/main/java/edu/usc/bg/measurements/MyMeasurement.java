	...
	public static String getSummary()
	{
		...
		try
		{
			...
			while(it.hasNext()){
				...
				double totalAvgWindowLatency = 0;
				long windowOps = 0;
				for(int i=0; i<allMeasurements.size(); i++){
						OpMeasurementTracker m = allMeasurements.get(i).data.get(opType);
						if(m!=null){
						if (m.getWindownumoperations()==0){
							windowOps=1;
						}
						else{
							windowOps= m.getWindownumoperations();
						}
							totalAvgWindowLatency +=( m.getWindowtotallatencysum()/windowOps);
						//	totalWindowOps += 
				...
				double report=totalAvgWindowLatency;
